...
extension RecoveryScorer {
    ...
    public static func recoveryTrace(hrv: Double,
        ...
        let rhrZForGuard: Double? = rhrB.flatMap { zScore($0.baseline, mean: rhr, spread: $0.spread) }
        ...
        guard let hrvZRaw = zScore(hrv, mean: hrvBaseline.baseline, spread: hrvBaseline.spread) else {
            lines.append("charge nilScore reason=hrvBaselineZeroVariance")
            return (nil, lines)
        }
        ...
        if let r = resp, let b = respBaseline, let z = zScore(b.baseline, mean: r, spread: b.spread) {
        ...
        if sat.active, let s = score, totalWeight > 0 {
            ...
            lines.append("charge saturation active hrvZraw=\(r2(hrvZRaw)) rhrZ=\(r2(rhrZForGuard ?? 0.0)) "
